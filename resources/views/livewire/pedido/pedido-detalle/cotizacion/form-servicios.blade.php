...
<div class="card col my-2 p-0">
    <div class="card-header col py-2 d-flex justify-content-between">
        ...
        <a style="cursor: pointer" wire:click="addServicio"><i style="font-size: 1.5rem; color:green"
                class="fas fa-plus"></i></a>
    </div>
    <div style="background: honeydew" class="card-body w-100 d-flex flex-column align-items-center">
        @foreach ($orderServicios as $index => $orderServicio)

            @livewire('pedido.pedido-detalle.cotizacion.detalle-servicios', ['orderServicio'=> $orderServicio , 'index' => $index], key($index))
            <div class="col-12 row d-flex justify-content-end">
                <a style="cursor: pointer" wire:click.prevent="removeServicio({{$index}})">
